...
※ 이미지는 Unsplash stock photo임. 수치·표·그래프는 이미지로 넣지 않고 본문 HTML 표로 처리함.
※ 이미지는 반드시 "실제 사진에 찍힐 수 있는 장면"만 묘사할 것.

- query: Unsplash 영어 검색어 2-4단어 (실제 사진 장면 기준, 구체적)
- alt: SEO 메타태그 — 주제 키워드 + 사진에 실제로 보이는 장면 묘사 (15-40자 한글)
  ✅ 좋은 예: "장기렌트 계약서에 서명하는 고객과 딜러", "자동차 딜러십 상담 창구 전경"
  ❌ 나쁜 예: "선수금 조건별 총비용 비교표", "월 납입금 계산 예시" (사진에 없는 데이터 설명 금지)
- caption: 이 사진이 본문 어느 맥락에서 등장하는지 독자에게 한 줄 설명 (15-35자 한글)
  ✅ 좋은 예: "계약서 서명 전 특약 조항을 꼼꼼히 확인하는 모습"
  ❌ 나쁜 예: "48개월 총납입액 비교" (데이터 설명 금지)
※ alt과 caption은 반드시 다른 내용이어야 함

예시:
  슬롯1 query:"car contract signing dealer" alt:"장기렌트 계약서 서명 전 딜러와 상담하는 고객" caption:"계약 전 특약 조항을 직접 확인하는 과정"
  슬롯2 query:"car rental showroom interior" alt:"자동차 렌탈 상담 센터 내부 전경" caption:"렌트사 방문 상담 시 꼭 챙겨야 할 포인트"
  슬롯3 query:"person reading car document" alt:"장기렌트 계약 서류를 검토하는 소비자" caption:"서류 검토 단계에서 숨겨진 비용을 잡아낼 수 있다"
